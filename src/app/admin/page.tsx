...
export default function AdminDashboard() {
  ...
  const weeklyDonations = useMemo(() => {
    ...
    const startOfWeek = today.subtract((dayOfWeek + 1) % 7, "day").startOf("day");
  
    // Get the upcoming Friday (end of the week)
    const endOfWeek = startOfWeek.add(6, "day").endOf("day");
    console.log("Start of Week (Saturday):", startOfWeek.format("YYYY-MM-DD HH:mm:ss"));
    console.log("End of Week (Friday):", endOfWeek.format("YYYY-MM-DD HH:mm:ss"));
  ...
  const monthlyData = useMemo(() => {
    ...
    const grouped = filteredDonations.reduce((acc, donation) => {
      ...
      <div className="flex h-screen w-full items-end justify-center bg-black/40 backdrop-blur-sm">
        <div className="mb-5 flex h-[81%] w-[90%] flex-col items-center justify-center text-center">
          <div className="flex h-full w-full">
            {/* Left Panel */}
            <div className="w-1/3 h-full p-5 flex flex-col gap-5 text-white">
              <div className="h-1/3 bg-black/50 text-primary font-bold text-2xl flex items-center justify-center rounded-lg">
                <p>Total Donations: <br /><br />₹{totalDonations.toLocaleString()}</p>
              </div>
              <div className="h-1/3 bg-black/50 text-primary font-bold text-2xl flex items-center justify-center rounded-lg">
                <p>Weekly Donations: <br /><br />₹{weeklyDonations.toLocaleString()}</p>
              </div>
              <div className="h-1/3 bg-black/50 text-primary font-bold text-2xl flex items-center justify-center rounded-lg">
                <p>DUMMY</p>
              </div>
            ...
            {/* Right Panel */}
            <div className="w-2/3 h-full flex flex-col text-white">
              {/* Graph Section */}
              <div className="h-[60%] bg-black/50 p-5 rounded-lg">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={monthlyData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    <XAxis dataKey="month" stroke="#EAC696" />
                    <YAxis stroke="#EAC696" />
                    <Tooltip cursor={{ fill: "rgb(0 0 0 / 0.5)" }} contentStyle={{ backgroundColor: "#222", borderRadius: "5px", color: "#EAC696" }} />
                    <Bar dataKey="donations" fill="#EAC696" radius={[5, 5, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
              {/* Empty Div Below Graph */}
              <div className="h-[40%] bg-black/50 text-primary mt-5 font-bold text-2xl flex items-center justify-center rounded-lg">
                <p>DUMMY</p>
              </div>
            </div>
          </div>
